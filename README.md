...
version of this file said the opposite, and said it as the single most important fact
about the project's status, so the correction gets the same place. The eleventh phone run
is the evidence, and `docs/STATUS.md` quotes the lines.

What that changes is what the next question is. It is no longer *"will an app start"* but
*"will an app that does not want to be virtualized start"* — which is a different problem
with different evidence, and is the one this pass began on. That question was then answered the only
way it can be: by reading the game rather than reasoning about it
([`docs/STANDOFF2.md`](docs/STANDOFF2.md)). It reads no `/proc` and no process list. It
reads four getters — `sourceDir`, `getApplicationInfo`, `getPackageCodePath`,
`nativeLibraryDir` — every one of which named `com.unique`. Those now report the paths an
installed copy of the app would report, and the paths resolve, because the file
interception was widened to the platform's own IO libraries. The `/proc` view stays and
reports what still leaks; it closes a real vector belonging to a different class of app.
...
|---|---|
| **Guests crash on a real Android 15 device** | Sixty-odd faults across sixteen phone logs. Each run moved the failure further down: the third launched nothing, the fourth launched everything and every guest was rendering in software, the fifth launched 6 of 8 and then **Play services killed three of them**, the sixth launched 6 of 7 with no launch fault at all, and the seventh and eighth launched everything they were given. What the sixth found was the *next* layer: a game with no expansion files, a game killed by UNIQUE's own PLT hook, and an app that died on a notification call nine milliseconds before the hook that covers it was installed. That one is a single fault with three victims: `GmsClient.getRemoteService` sends the guest's own package name to `com.google.android.gms`, which resolves the calling uid to UNIQUE's and answers `SecurityException: Unknown calling package name` on a `Handler`, where no app can catch it. The Google stack was hidden from every guest as a result — which the eighth run showed to be a worse fault than the crash, since a phone carrying GmsCore 26.32.34 was telling three apps it had no Google at all. That refusal is now **answered rather than avoided**: the request's calling package is rewritten to UNIQUE's own before it reaches Play services, which is the truth about the calling uid, so the bind succeeds and Maps, Firebase, ads, Dynamite and FCM work. Google **sign-in** is a separate question and not yet settled: one path answers `DEVELOPER_ERROR` because Google identifies an app by package *and signing certificate*, but the tenth run showed another path dying inside UNIQUE before it ever reached Google — an `Intent`'s extras had no class loader that could see the guest's own APK. That was fixed twice — the first fix could not have worked, because a `Bundle` binds its class loader at the first read of any key and UNIQUE always reads first — and the thirteenth run is the measurement: no crash, sign-in reaches Play services, and Play services refuses it there. That refusal is the ceiling, not a bug to find. The same log said there had never been a **keyboard** — `EditorInfo.packageName` is checked against the calling uid before an IME is bound, and a mismatch binds nothing, silently — and that a guest's own `FileProvider` never published, because providers were installed before the identity hooks and `attachInfo` runs the app's own code. "The screen does not respond" turned out not to be a touch fault at all: those were the dead windows the three crashes left behind. `docs/STATUS.md` has each fault with its log line. The two most recent runs each **disproved a fix from the run before** — a crash blamed on UNIQUE's PLT hook happened again with the hook removed, and a per-app Google rule turned out to be reading a constant — so both are now assertions in the analyzer rather than claims in a document. Several things here cannot be checked on the emulator at all, because it has no Play services, no IME, no `Android/obb` and no code-virtualization protector to break; those go straight to the phone. |
| **A published path the guest's own code cannot open** | Publishing installed-shaped paths is what closes the detection this project exists for, and the eighteenth run is what it costs when the redirect does not reach the caller: Standoff 2 showed *"Not enough storage space to install required resources"* because `libunity.so` could not open its own APK. Two causes, both fixed and both unverified: bionic's fortified `__open_2` was missing from the redirect table (and `__openat`, which is not a symbol, was in it), and the library-load watch was armed once rather than re-armed per load. The `paths` check reads the guest's own spelling of the complaint now, so a run in this state fails rather than passing. |
| **No Google flow is implemented** | `core/google` decides and records how each flow *would* be routed and reports `Unsupported` for every one. Sign-In, Credential Manager, Firebase and FCM have interfaces and no bodies. The sixth run settled one of them for good: a browser OAuth redirect **cannot** come back into a guest, because `myapp://callback` is declared by a package the platform has never installed and an intent filter is fixed at build time. `SIGN_IN` and `OAUTH_WEB` say so now instead of reporting `PASSTHROUGH`. **What changed after run 17** is what is known about the sign-in refusal: it was timed at 0.23–0.47 s across nine attempts in two logs, with no account picker ever drawn, which is not the OAuth-client wall this project had been describing. The request now leaves the space naming `com.unique` in the configuration Play services checks against the calling package — [`docs/GOOGLE_SIGN_IN.md`](docs/GOOGLE_SIGN_IN.md) §0 — and **no phone has run it yet**. |
| **A guest cannot sign in *as itself* with Google** — [`docs/GOOGLE_SIGN_IN.md`](docs/GOOGLE_SIGN_IN.md) is the full answer, and it is not the one that was assumed | Play services is visible to a guest again — an app whose client library is 18 or newer is told the truth about the device, and takes its native path instead of a browser. What that path returns is the unsolved half: Play services resolves the calling uid to UNIQUE, so a token comes back for UNIQUE's identity and not the app's, and an OAuth client registered for the app rejects it. Nothing UNIQUE can do from outside answers this — what Play services sees for a uid is decided inside Play services' own process. **In-space Google Play services is the architecture that does**, and it is the next large piece of work; `GoogleMode.VIRTUAL_GMS` and the `FORCE_VIRTUAL_GMS` flag are the shape of it and have no implementation behind them yet. |
| **A game's expansion files cannot be copied in automatically on Android 11+** | `MANAGE_EXTERNAL_STORAGE` — All-files access, the widest storage permission Android has — **explicitly does not cover `Android/data` or `Android/obb`.** No permission lets UNIQUE read another package's expansion files on a current release, and a build that said otherwise sent one user to grant an access that changed nothing. `GuestAssetImport` still copies what it *can* see, and what it cannot see it reports as nothing rather than as a fault. The route that works is the user handing the file over: **App Details → Files** browses the instance's own tree under the paths the guest uses, and **Import** copies into it from anywhere the system picker reaches. |
...
| **An app can still tell it is virtualized, and one of them was read to find out how** | Two answers, and they are not the same. **In general**: `/proc/self/maps` inside a `:vappN` names UNIQUE's APK and private directory, which is two lines and one `fopen`; that file is answered from a rewritten view for the guest's own native libraries, and the graft checks its own work and reports what still leaks (`PROC_VIEW_INSTALLED … leaked=0`). **For Standoff 2 specifically**, which is what this was built for, that closes nothing: the game reads no `/proc` at all. It reads `sourceDir` (×4), `getApplicationInfo` (×3), `getPackageCodePath` (×2) and `nativeLibraryDir` (×1) through JNI, puts the APK path into a signed `AppVerification` report, and sends it **as a field of `GoogleAuthRequest`** — so being refused a login and being told this is a virtual space are one event. [`docs/STANDOFF2.md`](docs/STANDOFF2.md) reads all of that out of the shipping binary. Those four getters now report `/data/app/~~<a>/<pkg>-<b>/…` and `/data/user/0/<pkg>`, and the paths resolve because the interception was widened to `libjavacore.so`, `libsqlite.so` and `libandroid_runtime.so` — every `java.io.File`, stream, preference and database a guest opens. The data half applies only after a probe proves the round trip on the device it is running on. **Not yet confirmed on a phone**: the change is host-tested and the next log is what settles it. What this does *not* close is the Google sign-in the report rides on — see the row below. |
| **A packed app whose protector refuses to register its natives** | `bin.mt.plus` loads `libmtprotect.so` cleanly and 58 ms later has no implementation for a class initialiser the library was supposed to provide. The protector ran and declined. UNIQUE's interception is now armed before `makeApplication`, so a protector loaded from the `Application` initialiser is patched before `JNI_OnLoad` — whether that changes its mind is a measurement, and until a log says so this app is still reported as `NO_APPLICATION`. |
...
| [`docs/STANDOFF2.md`](docs/STANDOFF2.md) | What one shipping game actually checks, read out of its own binary: the two "virtual space" messages, the report they come from, and the four getters that decide it |
...
| [`tools/device-log/README.md`](tools/device-log/README.md) | Reading a run from a phone: eighteen checks over a device log, no SDK and no device |
...
Eighteen checks, exit status 0 or 1, no SDK and no device: did every launch reach the guest's
