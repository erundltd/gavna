...
**Нашли.** Строка, которую я добавил в прошлую сборку, напечаталась — и назвала причину
прямо:
...
Движок игры (`libunity.so`) использует **две** функции из сорока одной, которые я
перехватываю: `realpath` и `readlink`. И ни одна из них файлы не открывает. В движке нет
вообще ни `open`, ни `stat`, ни `fopen` — **Unity обращается к файлам напрямую через
системные вызовы, минуя стандартную библиотеку.**

Поэтому все мои подмены путей были технически правильными и совершенно бесполезными
именно для этой библиотеки. И поэтому `libunity.so=2` четыре раунда подряд было верным и
бессмысленным числом, а я каждый раз угадывал, чем же Unity открывает файл.

Самое неприятное: эти данные были доступны в каждой проверке с самого начала — имя функции
лежит прямо в той же таблице, которую я обхожу. Просто никто их не печатал. Четыре прогона
на твоём телефоне ушли на угадывание того, что можно было прочитать.

### Что сделано

Теперь перехватывается и сам системный вызов, и путь в нём подменяется. Список номеров
вызовов, у которых есть путь, вынесен отдельно и покрыт тестами — потому что ошибиться тут
можно двумя способами, и оба дорогие:

- назвать путём аргумент, который путём не является — ядру уйдёт испорченное число;
- сделать хоть какую-то работу для вызова, у которого пути нет — а таких подавляющее
  большинство (блокировки потоков идут через тот же механизм), и лишняя работа там
  подвесила бы приложение.

Поэтому для всех остальных номеров — одно сравнение и передача дальше, без единого лишнего
действия.

### Что посмотреть

1. **Standoff 2 — пропала ли табличка про место.** Ради этого всё и делалось.
2. **Не появилось ли зависаний** в любом приложении: перехват теперь стоит перед очень
   частым системным вызовом. Если что-то стало виснуть — скажи, это будет от этой правки.
3. Если табличка пропала — дойди до меню и попробуй вход через Google.

## What changed one run ago

Прошлая правка сработала — но табличка осталась. Объясняю честно, что теперь известно и
чего ещё нет.
...
io_redirect: hooked 1 new slot(s) after loading …/libunity.so
io_redirect: hooked libunity.so=2
```

Игра не падает (значит подмена больше не ломает загрузку), загрузку движка теперь видно, и
до самого `libunity.so` перехват дотянулся. Обе стороны той проблемы, из-за которой игра то
падала, то не видела свой APK, закрыты.

### Чего не хватает

`libunity.so=2` — движок игры использует **две** из тридцати восьми функций, которые я
перехватываю. И лог не говорит, какие именно две и чем ещё движок открывает файлы. Из-за
этого последние несколько раундов я угадывал.

**Больше не угадываю.** Теперь в лог выводится строка с именами:

```
io_redirect: symbols libunity.so patched=<что перехвачено> unhooked=<файловые функции,
             которые движок использует, а я не перехватываю>
```

Второй список и есть ответ: функция, которую Unity вызывает, а я не подменяю — это и есть
та, которой он открывает свой APK. Добавить её — и всё.

Заодно добавил три функции, которых просто не было в списке (`freopen`, `freopen64`,
`statx`) — не наугад, а сверив мой список с настоящим списком функций Android. Есть шанс,
что одна из них и окажется нужной, и тогда табличка пропадёт уже в этой сборке.

### Что посмотреть

1. **Запусти Standoff 2 и пришли лог** — даже если табличка осталась. В нём впервые будет
   строка `io_redirect: symbols libunity.so …`, которая называет недостающую функцию прямо.
2. Если таблички нет — вход через Google: появляется ли список аккаунтов.

## What changed two runs ago

Игра теперь запускается — падения больше нет. И сразу вернулась старая табличка про место.
Это не две разные проблемы, а две стороны одной, и теперь понятно, как закрыть обе.
...
Самое важное — не эта строка, а то, чего в логе **нет**. Есть `libmain.so=0` (это
загрузчик Unity), а `libunity.so` нет вообще, и нет ни одной строки «перехватил после
загрузки». То есть до самого движка игры перехват не дотянулся, и открыть свой APK он не
может.

### Почему так вышло

В прошлой сборке я убрал перехват `dlopen`, потому что из-за него игра падала. Убрал —
падение ушло, но вместе с ним ушло и уведомление о загрузке: Unity грузит свой движок
именно через `dlopen`, и никто этого больше не видел.

То есть:

- **перехват включён** → система думает, что библиотеку запрашивает UNIQUE, и не находит
  её по имени → игра падает (прошлый лог);
- **перехват выключен** → никто не видит загрузку движка → движок не может открыть свой
  APK → табличка про место (этот лог).

### Как закрыто

Оказалось, есть третий вариант — тот, который делает сама система. Функция `dlopen` внутри
Android состоит из одной строки: она передаёт загрузчику **адрес того, кто её вызвал**.
Единственное, что ломает перехват, — именно этот адрес. Теперь перехват вызывает
внутреннюю функцию загрузчика напрямую и передаёт ей настоящего вызывающего (Unity, а не
UNIQUE). Пространство имён остаётся на месте, путь подменяется, и загрузка становится
видна.

Если по какой-то причине эту внутреннюю функцию найти не удастся, перехват `dlopen`
просто не включится — падение хуже, чем табличка.

### Что посмотреть

1. **Standoff 2 — запускается ли (не падает) и что с табличкой про место.**
2. Если табличка осталась — пришли лог всё равно: он теперь впервые назовёт конкретную
   функцию, которой не хватает, а не оставит гадать. В логе должна появиться строка вида
   `hooked libunity.so=<число>`.
3. Если таблички нет — вход через Google: появляется ли список аккаунтов.

## What changed three runs ago

Игра вылетает — и это моя ошибка, внесённая две сборки назад. Нашёл, исправил.
...
Abort message: 'JNI FatalError called: Unable to load library:
  ...
  → com.unity3d.player.UnityPlayer.loadNative
  → com.unity3d.player.UnityPlayerActivity.onCreate
```

Игра падает в самом начале, ещё до запуска своего движка. Обрати внимание, на что ругается
система: не на путь из сообщения, а на **имя** `libunity.so`.

В Android имя библиотеки ищется в «пространстве имён» **той библиотеки, которая её
запрашивает** — система смотрит, кто именно вызвал загрузку. Я перехватывал этот вызов, и
из-за перехвата получалось, что запрашивает его UNIQUE, а не сам Unity. В списке путей
UNIQUE библиотеки игры, естественно, нет — отсюда «not found».

Сам перехват стоял давно. Но две сборки назад я исправил другую вещь — включил
отслеживание загрузки библиотек не один раз, а постоянно — и именно из-за этого перехват
впервые дотянулся до загрузчика Unity. То есть я починил одно и сломал другое.

**Исправлено:** перехват `dlopen` убран совсем. Второй, безопасный путь загрузки
(`android_dlopen_ext`, через который идёт `System.loadLibrary`) остаётся — там система
получает нужное пространство имён явным параметром, и подмена ей не мешает.

### И то же самое было в прошлом логе, а я не увидел

В логе от 17:25 эти же два падения уже были. Мой анализатор их не показал: он читал только
падения Java, а это падение уровнем ниже — оно оставляет другой вид записи. Теперь читает
оба, и сообщение об ошибке сохраняется целиком, потому что самое важное в нём — в конце
(какая библиотека и почему).

Это уже третий раз за четыре прогона, когда проверка говорила «всё хорошо», а у тебя на
экране была ошибка. Каждый раз по одной и той же причине: приложение пишет о сбое не тем
способом, который анализатор искал. Каждый такой случай теперь закрыт тестом на твоём же
логе.

### Что посмотреть

1. **Standoff 2 — запускается ли вообще и доходит ли до меню.** Это главное.
2. Если дошла — есть ли ещё табличка про нехватку места.
3. Если и её нет — вход через Google: появляется ли список аккаунтов.
...
GUEST_PATHS_PUBLISHED … code=true sqlite=8 data=true
```

`data=true` — **впервые за всю историю проекта**. Обе половины того, что игра о себе
сообщает (путь к APK и папка данных), теперь публикуются и реально работают: и запись
через `java.io`, и открытие базы данных попадают внутрь инстанса.

И тот же лог подтвердил мой прошлый вывод дословно: `libsqlite.so=2+abs+packed` — метка
`+packed` стоит на **всех** системных библиотеках. Правка через релокации, которую сделала
прошлая нейросеть, физически не могла сработать.

### Почему при этом игра написала про нехватку места
...
Игре сообщили «правильный» путь к её собственному APK — и **её собственный движок не смог
его открыть**. С точки зрения игры её файлы пропали, поэтому она и говорит, что не хватает
места для установки ресурсов. В 16-м прогоне (когда пути ещё не публиковались) этой ошибки
нет вообще — значит она появилась вместе с публикацией. И, честно говоря, она была уже в
предыдущем логе, четыре раза, а я её не увидел, потому что моя же проверка её не искала.

Две причины, обе проверены не рассуждением, а командой по системной библиотеке Android:

1. **`__open_2`.** Когда приложение собрано в релизе (а Unity собрана), вызов
   `open(путь, только-чтение)` компилируется **не** в `open`, а в отдельную функцию
   `__open_2`. Её в таблице перехвата не было. Зато там был `__openat` — имя, которого в
   Android вообще не существует; его написали по памяти. В логе оба случая выглядели
   одинаково («это имя никто не использует»), поэтому ошибка прожила несколько сборок.
2. **Слежение за загрузкой библиотек включалось один раз.** Unity грузит `libunity.so` не
   через систему, а сама из `libmain.so` — и этот момент не отслеживался, поэтому
   `libunity.so` в твоём логе вообще ни разу не перехватывалась.

Обе исправлены. Плюс добавлена проверка, которая сверяет **каждое** имя в таблице
перехвата с настоящей системной библиотекой Android — та самая, что поймала бы `__openat`
сразу. И анализатор логов теперь ловит жалобу движка игры («Unable to open …»), а не только
системную — раньше он на этом логе показывал «всё хорошо», пока у тебя на экране была
ошибка.

### Что посмотреть в этот раз, по порядку

1. **Просто запусти Standoff 2.** Главный вопрос: **пропала ли табличка про нехватку
   места?** Если она осталась — скажи сразу, лог теперь называет библиотеку, до которой
   перехват не дотянулся, и следующий шаг будет однозначным.
2. **Открой пару приложений с данными** — файловый менеджер, что-то, где ты был залогинен.
   Папка данных теперь тоже подменяется, и это первый раз, когда это доехало до телефона.
3. **Вход через Google** (если игра дошла до меню). Появляется ли **список аккаунтов**? Это
   правка из прошлой сборки, её ещё никто не проверял.
...
## What changed thirteen runs ago
...
## What changed fourteen runs ago
