...
105 tests, under a second, no dependencies. Three kinds:
...
- **`fixtures/redmi-android15-run4.log` … `-run16.log`** — the runs after it,
  ...
  failing on the guest's own data path. The sixteenth is where the per-library diagnostic
  paid: `hooked libjavacore.so=8` beside a gate refusing to publish is what said
  `java.io.File` is two libraries and only one of them was hooked.
